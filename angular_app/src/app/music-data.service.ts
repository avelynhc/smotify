...
export class MusicDataService {
  ...
  getAlbumsByArtistId(id:any): Observable<SpotifyApi.ArtistsAlbumsResponse> {
    return this.spotifyToken.getBearerToken().pipe(mergeMap(token=> {
      ...
      return this.http.get<SpotifyApi.ArtistsAlbumsResponse>(`https://api.spotify.com/v1/artists/${id}/albums`, {params, headers: { "Authorization": `Bearer ${token}` }})
    }));
  }
  ...
  searchArtists(searchString:any): Observable<SpotifyApi.ArtistSearchResponse> {
    return this.spotifyToken.getBearerToken().pipe(mergeMap(token=> {
      ...
      return this.http.get<SpotifyApi.ArtistSearchResponse>(`https://api.spotify.com/v1/search`, {params, headers: { "Authorization": `Bearer ${token}` }});
    }));
  }
    ...
    return this.http.delete<[String]>(`${environment.userAPIBase}/favourites/${id}`).pipe(mergeMap(favouritesArray => {
      return this.spotifyToken.getBearerToken().pipe(mergeMap(token=> {
        ...
        if (favouritesArray.length > 0) {
          return this.http.get<any>("https://api.spotify.com/v1/tracks", {params, headers: { "Authorization": `Bearer ${token}` }});
        };
